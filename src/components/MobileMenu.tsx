  ...
  return (
    <>
      ...
      <button
        ...
        className="relative z-[110] flex md:hidden items-center justify-center w-11 h-11 rounded-full bg-surface border border-border"
      ...
      >
        <div className="relative w-[18px] h-[14px]">
          ...
          <motion.span
            ...
            className="absolute top-1/2 left-0 w-full h-[2px] rounded-full bg-white -translate-y-1/2"
          />
          {/* Bottom line */}
          <motion.span
            animate={open ? { rotate: -45, y: -5 } : { rotate: 0, y: 0 }}
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 z-[var(--z-overlay)] bg-black/80 backdrop-blur-2xl"
              onClick={close}
            />
              ...
              <nav className="flex flex-col items-center gap-5 w-full max-w-sm pointer-events-auto">
                {links.map((link, i) => (
                  <motion.div
                    ...
                    exit={reduce ? undefined : { opacity: 0, y: -16, filter: "blur(4px)", transition: { delay: (links.length - 1 - i) * 0.04 } }}
                    transition={{
                      duration: 0.5,
                      delay: i * 0.06,
                      ease: [0.32, 0.72, 0, 1],
                  ...
                  >
                    <Link
                      href={link.href}
                      onClick={close}
                      className="block text-center py-4 text-2xl font-bold text-white/80 hover:text-white transition-colors active:scale-[0.97]"
                      style={{ minHeight: 44 }}
                    >
                      {link.label}
                    </Link>
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  transition={{ duration: 0.5, delay: links.length * 0.06, ease: [0.32, 0.72, 0, 1] }}
